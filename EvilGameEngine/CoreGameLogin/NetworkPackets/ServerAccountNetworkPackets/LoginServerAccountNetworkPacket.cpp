...
#include "LoginServerAccountNetworkPacket.h"
...
LoginServerAccountNetworkPacket::LoginServerAccountNetworkPacket( const BinaryStream* datastream) :
...
LoginServerAccountNetworkPacket::LoginServerAccountNetworkPacket( uint32 requiredVersion, const String& gameKey, Account* account) :
...
BinaryStream* LoginServerAccountNetworkPacket::GetDataStream()
{
...
bool LoginServerAccountNetworkPacket::CanExecute()
{
...
void LoginServerAccountNetworkPacket::Execute()
{
  ...
  if (loginModule != NULL)
  {
    ...
    uint32 errorCode = loginModule->LoginAccount( m_Account );
    if ( errorCode == GameEnginePacketData::ErrorCode_OK )
    {
      loginModule->AddOnlineAccount( m_Account, m_ConnectionId );
      SendPacketToClient( new ClientAccountLoggedOnNetworkPacket( m_Account ) );
    }
    else
    {
      SendLoginErrorToClient(GameLoginPacketData::PacketData_LoginServerAccount, errorCode);
    }
  }
  else
  {
    SendLoginErrorToClient(GameLoginPacketData::PacketData_LoginServerAccount, GameLoginPacketData::ErrorCode_BrokenAccountData);
  }
}
