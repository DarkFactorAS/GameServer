...
if not "%DF_EXTERNALLIBS%" == "" (
  ...
  set MYSQL=%DF_EXTERNALLIBS%\mysql\mysql-connector-1.1.3
  set OPENSSL=%DF_EXTERNALLIBS%\openssl-1.0.0.e
  set GLEW=%DF_EXTERNALLIBS%\glew
  set CURL=%DF_EXTERNALLIBS%\libcurl-7.36.0

  set LINKER=VS2015
  set INCLUDEDIR=%DF_EXTERNALLIBS%\mysql\mysql-connector-1.1.3\include;%DF_EXTERNALLIBS%\boost-1.59.0\include\;%DF_EXTERNALLIBS%\glew\include;%DF_EXTERNALLIBS%\zlib-1.2.5;%DF_EXTERNALLIBS%\openssl-1.0.0.e\include
  set PREPROCESSOR=CLIENT;WIN32;_CONSOLE;BOOST_ALL_NO_LIB;CPPDBC_PUBLIC_FUNC;USE_OPENSSL

  Start "EvilGameEngine Unittest" Windows\EvilGameEngineUnitTest.sln

  REM Start the p4 operation after we have started Visual Studio to make it "optional"
  p4 changes -m1 //depot/Multiplatform/EvilEngine/... > ..\..\EvilEngine\CoreVersion\versiontmp.txt
