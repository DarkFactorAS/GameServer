...
#elif defined PLATFORM_OSXCLIENT
  // TOOD: Should do this in xcode as well
#else
  #error "Undefined OSX varient"
#endif  
...
String dfAssertHandler::DumpCallstack( bool returnString )
{
  ...
  if ( count > MAX_CALLSTACK_LENGTH )
  {
    count = MAX_CALLSTACK_LENGTH;
  }
...
void dfAssertHandler::HandleSystemAssert(int signalNumber, siginfo_t *sigInfo, void *context) 
{
  ...
  // Application just got a normal shutdown signal
...
#ifdef PLATFORM_OSXSERVER
    raise(SIGTRAP);
#elif defined PLATFORM_OSXCLIENT
    // TODO: Find the signtrap equivivalent to stop in the xcode debugger
#else
  #error "Undefined OSX variant"
#endif
